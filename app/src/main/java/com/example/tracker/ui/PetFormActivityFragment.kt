...
class PetFormActivityFragment : Fragment() {
    ...
    private fun savePetToRoom(root: View) {
        val name = root.findViewById<TextInputEditText>(R.id.etPetName)
            ?.text?.toString()?.trim().orEmpty()

        val type = root.findViewById<TextInputEditText>(R.id.etPetType)
            ?.text?.toString()?.trim().orEmpty()

        val breed = root.findViewById<TextInputEditText>(R.id.etBreed)
            ?.text?.toString()?.trim().orEmpty()

        val birthDateStr = root.findViewById<TextInputEditText>(R.id.etBirthDate)
            ?.text?.toString()?.trim().orEmpty()

        val gender = root.findViewById<AutoCompleteTextView>(R.id.actvGender)
            ?.text?.toString()?.trim().orEmpty()
        ...
        //  Run Room suspend insert in a coroutine (IO thread)
